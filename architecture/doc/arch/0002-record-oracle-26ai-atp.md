...
Migrate from **Oracle 19c ATP** to **Oracle 26ai ATP** as the sole database for all relational and vector data.

The ONNX model `ALL_MINILM_L12_V2` is loaded directly into the Oracle 26ai engine via `DBMS_VECTOR.LOAD_ONNX_MODEL`. A database trigger (`trg_task_embedding_bu`) automatically generates and stores the embedding for every task title on INSERT or UPDATE, using:
